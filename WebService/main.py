...
from .data.prediction_handler import get_predictions
...
async def predict(file: UploadFile = File(...)):
    ...
    try:
        ...
        image = np.asarray(Image.open(io.BytesIO(contents)).convert('RGB'))
        width, height = image.shape[:2]
        logger.info(f"Got {width}x{height} image, prediction to come...")
        ...
        company, breakdown = preds
        formatted_breakdown = []
        
        for (name, percentage) in breakdown.items():
            formatted_breakdown.append(ResultsItem(company=name, percentage=percentage*100))
        ...
        return Classification(
            ...
            predictedCompany=company,
            predictionConfidence=breakdown[company],
            classifications=formatted_breakdown
        )
